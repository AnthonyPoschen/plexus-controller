...
func (r *GameServerReconciler) reconcileRunning(ctx context.Context, gameServer *plexusv1alpha1.GameServer, definition games.GameDefinition) (ctrl.Result, error) {
	...
	deployment, err := r.ensureDeployment(ctx, gameServer, definition)
	if err != nil {
...
func validateDesiredState(gameServer *plexusv1alpha1.GameServer) error {
	...
	if gameServer.Spec.SelectedSetup.Configuration.SchemaVersion != factorio.SchemaVersion {
		return fmt.Errorf("Factorio setup schemaVersion is %q, want %q", gameServer.Spec.SelectedSetup.Configuration.SchemaVersion, factorio.SchemaVersion)
	}
...
func environment(definition games.GameDefinition) []corev1.EnvVar {
